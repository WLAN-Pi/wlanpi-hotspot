...
SEC
    ...
    sed -i "s/^INTERFACESv4=.*$/INTERFACESv4=\"$INTERFACE\"/" $ISC_DHCP_FILE
...
#####################################################
 ...
 with no errors, you may now switch in to hotspot
 mode.

 Would you like me to switch your WLAN Pi in to
 hotspot mode (this will cause a reboot)?

##################################################### 
COMPLETE
    
    read -p "Would to like switch to hotspot mode? (y/n) : " yn

    case $yn in
        y|Y ) echo "Switching...";;
        *   ) echo "OK, you can switch to hotspot mode later using the front panel buttons. We're all done. Bye!"; exit 0;
    esac

    echo "(After a reboot, the WAN Pi will come back up in hotspot mode.)"
    /usr/sbin/hotspot_switcher on
